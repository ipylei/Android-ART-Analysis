...
【先来了解ART虚拟机所定义的那六个Runtime ArtMethod 对象】。
*/
//[runtime.h]
class Runtime{
    ......
...
//我们先来了解上述代码中最后所列的三个Runtime Method对象。它们的设置都在 ClassLinker 的InitWithoutImage函数中，代码如下所示。
...
ArtMethod* Runtime::CreateResolutionMethod() {
    ...
    if (IsAotCompiler()) {
        ...
        // resolution_method_ 的机器码入口地址为 art_quick_resolution_trampoline，它对
        // 应的跳转目标为 artQuickResolutionTrampoline 函数。
...
//【10.1.2.2】　ImtConflictMethod和ImtUnimplementedMethod
...
ArtMethod* Runtime::CreateImtConflictMethod(LinearAlloc* linear_alloc) {
    ...
    //创建一个 ImtConflictTable 对象，并将这个对象的地址赋值给 method的 jni机器码 入口
    method->SetImtConflictTable(class_linker->CreateImtConflictTable(0u, linear_alloc), pointer_size);
    return method;
}
/*
也就是说，对imt_conflict_method_和imt_unimplemented_method_这两个Runtime Method对象而言：
    ·它们的机器码入口地址都是 art_quick_imt_conflict_trampoline。
    ·它们的jni机器码入口地址实际上是一个 ImtConflictTable 对象。
*/
...
//接下来了解Runtime类中另外三个Runtime Method，它们和 CalleeSaveType 有关。创建它们的代码如下。
...
/*
CalleeSaveType相关的三个Runtime Method对象的机器码入口地址为nullptr，说明这三个对象并不关联任何跳转代码或跳转目标。那它们的作用是什么呢？
原来，这三个Runtime Method对象主要用于其他跳转代码针对不同情况的参数传递、栈回溯等工作。
