...
//ART中还有一个DexFileReference，其代码如下所示。
//值得指出的是，它的index成员变量的含义与MethodReference dex_method_index的含义一样，代表某个Java方法在dex文件里method_ids中的索引。
...
class VerifiedMethod {
    ...
    private:
        ...
        ......
        DevirtualizationMap devirt_map_;  //和函数调用的去虚拟化有关
...
void VerifiedMethod::GenerateDevirtMap(verifier::MethodVerifier* method_verifier) {
    ...
    /*method_verifier 类型为 MethodVerifier，它包含了已经校验通过的Java方法的信息。
      CodeItem 函数返回这个Java方法对应的dex字节码。CodeItem类对应第3章图3-8中的code_item伪数据结构。
    */
    ...
    for (; inst < end; inst = inst->Next()) {
        ...
        const bool is_range = .....;
        //VRregC_35c是ART提供的获取指令中指定虚拟寄存器值的辅助函数。reg_type存储了该虚拟寄存器的值。
        //对invoke-virtual/interface调用指令来说，它代表在哪个对象上发起了函数调用
...
//对QuickCompilerCallbacks类而言，只需要关注它的MethodVerified函数即可，来看代码。
//[quick_compiler_callbacks.cc->QuickCompilerCallbacks::MethodVerified]
void QuickCompilerCallbacks::MethodVerified(verifier::MethodVerifier* verifier) {
    //调用VerificationResults的 ProcessVerifiedMethod 函数。如上文所述，VerificationResults用来保存和Java方法校验相关的信息，
    //详情见下文代码分析。
    verification_results_->ProcessVerifiedMethod(verifier);
    
    MethodReference ref = verifier->GetMethodReference();
    
    /*method_inliner_map_ 的类型为 DexFileToMethodInlinerMap，GetMethodInliner 函数返回值的类型为 DexFileMethodInliner。
      它和编译器做内联优化有关。目前dex2oat只能优化包含最多两个操作指令的方法。请读者自行阅读相关代码。
    */
